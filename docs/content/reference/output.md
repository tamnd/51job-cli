...
describes what users get. Pick a format with `-o`, or let job51 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
job51 <command> -o table   # aligned columns for reading
job51 <command> -o jsonl   # one JSON object per line, for piping
job51 <command> -o json    # a single JSON array
job51 <command> -o csv     # spreadsheet friendly
job51 <command> -o tsv     # tab-separated
job51 <command> -o url     # just the URL column
job51 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
job51 <command> --fields id,title,url
```
...
```bash
job51 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
job51 <command>            # a table, because this is a terminal
job51 <command> | wc -l    # JSONL, because this is a pipe
```
